...
void Server::cmdWHO(const int& socket, const t_message* message)
{
	...
	if (message->arguments[0].empty())
	{
		...
		sendMessage(socket, std::string(":") + SERVER_ADDRESS " " + RPL_ENDOFWHO + " " + client.nick + " :End of /WHO list\r\n");
		return;
	}
	...
	if (message->arguments[0][0] == '#' || message->arguments[0][0] == '&')
	{
		...
		if (channel)
		{
			...
			{
				sendMessage(socket, std::string(":") + SERVER_ADDRESS " " + RPL_WHOREPLY + " " + client.nick + " " + channel->channelName + " " + it->first->user + " " + it->first->hostname + " localhost " + it->first->nick + " H :0 " + it->first->realname + "\r\n");
			}
		}
	}
	else // procurar nickname
	{
		...
		{
			// reply RPL_WHOREPLY
			sendMessage(socket, std::string(":") + SERVER_ADDRESS " " + RPL_WHOREPLY + " " + client.nick + " * " + user->nick + " " + user->user + " " + user->hostname + " localhost " + user->nick + " H :0 " + user->realname + "\r\n");
		}
	}
	// reply RPL_ENDOFWHO
	sendMessage(socket, std::string(":") + SERVER_ADDRESS " " + RPL_ENDOFWHO + " " + client.nick + " " + message->arguments[0] + " :End of /WHO list\r\n");
}
