...
void Server::cmdPRIVMSG(const int& socket, const t_message* message)
{
	...
	if (message->arguments[0].empty())
	{
		...
		sendMessage(socket, std::string(":") + SERVER_ADDRESS + " " + ERR_NORECIPIENT + " " + client.nick + " PRIVMSG :No recipient given (PRIVMSG)\r\n");
		return;
	}
	...
	if (message->arguments[1].empty())
	{
		...
		sendMessage(socket, std::string(":") + SERVER_ADDRESS + " " + ERR_NOTEXTTOSEND + " " + client.nick + " :No text to send\r\n");
		return;
	}
	...
	if (message->arguments[0][0] == '#' || message->arguments[0][0] == '&')
	{
		...
		sendMessage(socket, std::string(":") + SERVER_ADDRESS + " " + ERR_CANNOTSENDTOCHAN + " " + client.nick + " " + message->arguments[0] + " :Cannot send to channel\r\n");
		return;
	}
