...
void Server::cmdMODE(const int& socket, const t_message* message)
{
	...
	{
		sendMessage(socket, std::string(":") + SERVER_ADDRESS " " + ERR_NEEDMOREPARAMS + " " + client.nick + " MODE :Not enough parameters\r\n");
		return;
	}
	...
	if (message->arguments[0][0] == '#' || message->arguments[0][0] == '&')
	{
		...
		{
			std::string reply;
			reply = std::string(":") + SERVER_ADDRESS " " + RPL_CHANNELMODEIS + " " + client.nick + " " + channel->channelName + " ";
		...
		if (i == channel->userList.end() || (i->second & MODE_o) == 0)
		{
			...
			sendMessage(socket, std::string(":") + SERVER_ADDRESS " " + ERR_CHANOPRIVSNEEDED + " " + client.nick + " " + channel->channelName + " :You're not channel operator\r\n");
			return;
		}
		// iterar pelos modos
		bool setMode = true;
		...
		for (size_t i = 0; i < message->arguments[1].length(); i++)
		{
			...
			case 'o':
				...
				if (setMode && argIndex < 15 && !message->arguments[argIndex].empty())
				{
					...
					if (clientInChannel == channel->userList.end())
					{
						...
						sendMessage(socket, std::string(":") + SERVER_ADDRESS " " + ERR_USERNOTINCHANNEL + " " + client.nick + " " + message->arguments[argIndex] + " " + channel->channelName + " :They aren't on that channel\r\n");
						continue;
					}
			...
			default:
				...
				sendMessage(socket, std::string(":") + SERVER_ADDRESS " " + ERR_UMODEUNKNOWNFLAG + " " + client.nick + " :Unknown MODE flag\r\n");
				break;
			}
		}
	...
	else // procurar nickname
	{
		...
		{
			std::string reply;
			reply = std::string(":") + SERVER_ADDRESS " " + RPL_UMODEIS + " " + client.nick + "\r\n";
