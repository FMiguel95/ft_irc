...
void Server::cmdTOPIC(const int& socket, const t_message* message)
{
	...
	if (message->arguments[0].empty())
	{
		...
		sendMessage(socket, std::string(":") + SERVER_ADDRESS + " " + ERR_NEEDMOREPARAMS + " " + client.nick + " TOPIC :Not enough parameters\r\n");
		return;
	}
	...
	if (message->arguments[1].empty() && message->raw[message->raw.length() - 1] != ':')
	{
		...
		if (channel->topic.empty())
			sendMessage(socket, std::string(":") + SERVER_ADDRESS + " " + RPL_NOTOPIC + " " + client.nick + " " + channel->channelName + " :No topic is set\r\n");
		else
			sendMessage(socket, std::string(":") + SERVER_ADDRESS + " " + RPL_TOPIC + " " + client.nick + " " + channel->channelName + " :" + channel->topic + "\r\n");
		return;
	}
