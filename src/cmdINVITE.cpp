...
void Server::cmdINVITE(const int& socket, const t_message* message)
{
	...
	if (!channel)
	{
		...
		sendMessage(socket, std::string(":") + SERVER_NAME + " " + ERR_NOSUCHCHANNEL + " " + client.nick + " " + message->arguments[1] + " :No such channel\r\n");
		return;
	}
	...
	sendMessage(client.socket, std::string(":") + SERVER_NAME + " " + RPL_INVITING + " " + client.nick + " " + message->arguments[0] + " " + message->arguments[1] + "\r\n");
