...
int Server::error_exit(const char *message)
{
...
int Server::runServer()
{
	...
	while (run)
	{
		...
		if (fds[0].revents & POLLIN)
		{
			...
			std::cout << "\001\e[0;32m" << "New client connected: " << "\e[0m\002" << inet_ntoa(clientAddress.sin_addr) << ':' << ntohs(clientAddress.sin_port) << std::endl;
		}
		...
		for (size_t i = 1; i < fds.size(); i++)
		{
			if (fds[i].revents & POLLIN)
			{
				...
				if (bytesRead <= 0)
				{
					std::cout << "\001\e[0;31m" << "Client " << fds[i].fd << " disconnected" << "\e[0m\002" << std::endl;
			...
			{
				std::cerr << "\001\e[0;31m" << "Client " << fds[i].fd << " poll error\e[0m\002" << std::endl;
				Client& client = _clients.at(fds[i].fd);
				if (client.isRegistered)
					unregisterClient(client, "Client Quit");
				_clients.erase(fds[i].fd);
...
void Server::checkTimeouts(std::vector<pollfd>& fds)
{
	...
	for (size_t i = 1; i < fds.size(); i++)
	{
		...
		else if (!client.isRegistered && diff > TIMEOUT_TIME)
		{
			...
			std::cout << "client " << client.socket << ": disconnected unregistered client" << std::endl;
			_clients.erase(fds[i].fd);
			close(fds[i].fd);
			fds.erase(fds.begin() + i);
			i--;
...
Channel* Server::getChannelByName(const std::string& name)
{
	...
	{
		if (name.length() != it->channelName.length())
			continue ;
		for (size_t i = 0; i < name.length(); i++)
		{
			if (std::tolower(name[i]) != it->channelName[i])
				continue;
		}
		return (Channel*)&*it;
	}
